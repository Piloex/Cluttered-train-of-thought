# -*- coding: utf-8 -*-
# @Time:2023年07月21日 2:45
# @Author:Piloex
# @Email:DistBelief@qq.com
# @File:PatchMix.py
# @Project :PatchMix
...
class PatchMixer:
    ...
    def __call__(self, X, target):
        N = X.shape[0]
        m = np.random.choice(self.mix_n) if isinstance(
            self.mix_n, list) else self.mix_n
        use_mix = np.random.rand() < self.mix_p and m > 1
        if use_mix:
            patch_size = np.random.choice(self.mix_s) if isinstance(
                self.mix_s, list) else self.mix_s

            patches = rearrange(
                X.unsqueeze(0),
                'b c (w p1) (h p2) -> b (w h) (c p1 p2)', p1=patch_size, p2=patch_size)
        ...
        soft_mix_target = self._one_hot(
            mix_target, self.num_classes, on_value.expand(
                [mix_target.shape[0], -1]).cuda(),
            off_value)
...
patch_mixer = PatchMixer(num_classes, mix_s, mix_n, mix_p, smoothing)
target = torch.tensor(1)
processed_image, soft_target, soft_mix_target = patch_mixer(
    image_tensor, target)
